...
Bullgram использует работы следующих проектов. Лицензии каждого — в соответствующих репозиториях.
...
- Bullgram содержит **патчи** поверх upstream в `userbot-web/patches/` — они распространяются под **AGPL-3.0** как часть этого репо.
- Совместимость: GPL-3.0 совместим с AGPL-3.0 — патчи и upstream могут жить в одном дистрибутиве.
- Upstream не коммитится в Bullgram-репо (см. `.gitignore`); для локальной работы нужен отдельный clone upstream (см. `userbot-web/README.md`).

## Supabase self-host

Bullgram запускает self-hosted Supabase. Состав Supabase распространяется под смешанными лицензиями (Apache-2.0 / FreeBSD / PostgreSQL). Полный attribution — на [supabase.com/legal](https://supabase.com/legal).

## Иконки и asset'ы

- Lucide icons (ISC) — см. выше
- Логотипы и бренд-asset'ы Bullgram — авторские, разрешены к использованию только как часть дистрибутива Bullgram под AGPL-3.0.

## License summary

Код Bullgram — под [AGPL-3.0](./LICENSE). Все вышеперечисленные third-party работы остаются под их собственными лицензиями.
