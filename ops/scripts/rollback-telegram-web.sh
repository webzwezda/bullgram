...
#   3) Static artifact wipe — rm /var/www/bullgram-telegram-web/. Last
...
STATIC_DIR="/var/www/bullgram-telegram-web"
...
case "$ACTION" in
    status)
        ...
        ssh "$SERVER" bash -s <<'REMOTE'
            ...
            if [ -d /var/www/bullgram-telegram-web ] && [ -f /var/www/bullgram-telegram-web/index.html ]; then
                echo "  present ($(ls /var/www/bullgram-telegram-web | wc -l) files)"
            else
                echo "  MISSING"
            fi
            echo
...
REMOTE
    ...
    hard)
        ...
        ssh "$SERVER" bash -s <<'REMOTE'
            if [ -d /var/www/bullgram-telegram-web ]; then
                mv /var/www/bullgram-telegram-web /var/www/bullgram-telegram-web.disabled.$(date +%s)
                echo "Moved to /var/www/bullgram-telegram-web.disabled.*"
            else
...
REMOTE
    ...
    restore)
        ...
        ssh "$SERVER" bash -s <<'REMOTE'
            ...
            LATEST=$(ls -1d /var/www/bullgram-telegram-web.disabled.* 2>/dev/null | head -1 || true)
            if [ -n "$LATEST" ] && [ ! -d /var/www/bullgram-telegram-web ]; then
                mv "$LATEST" /var/www/bullgram-telegram-web
                chown -R www-data:www-data /var/www/bullgram-telegram-web
