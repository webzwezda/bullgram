...
export function CreateInvoicePage() {
  ...
  const validate = () => {
    ...
    if (!form.secret_payload) next.secret_payload = 'Укажите секрет';
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      <div className="mx-auto w-full max-w-2xl px-4 py-6 sm:py-10">
        <header className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              ...
              <div className="text-[10px] text-slate-400 uppercase tracking-wider">Продажа без регистрации</div>
            </div>
          </div>
        </header>
        ...
        <div className="rounded-3xl bg-white border border-slate-200 p-6 sm:p-8 shadow-sm">
          <div className="mb-6">
            <h1 className="text-2xl font-black tracking-tight text-slate-900">Создать счёт на оплату</h1>
            <p className="text-sm text-slate-500 mt-1">
              Заполните условия — получите ссылку для покупателя. Без аккаунта, оплата в TON, TTL 1.5 часа.
            </p>
          </div>

          <form onSubmit={onSubmit} className="space-y-7">
            <section className="space-y-3">
              <Label icon={Tag}>Что продаёте</Label>
              <input
                type="text"
                value={form.title}
                onChange={(e) => update('title', e.target.value.slice(0, 120))}
                onBlur={() => form.title && validate()}
                placeholder="Steam ключ 50 USD"
                maxLength={120}
              ...
              <textarea
                ...
                placeholder="Активация на store.steampowered.com (необязательно)"
                maxLength={500}
                rows={2}
                aria-invalid={!!errors.description}
                className={`w-full rounded-xl border bg-white px-3.5 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/5 transition-colors ${errors.description ? 'border-rose-300' : 'border-slate-200 focus:border-slate-400'}`}
              />
              <FieldError message={errors.description} />
            </section>

            <section className="space-y-3">
              <Label icon={Wallet}>Сколько (TON)</Label>
              <input
                type="number"
                inputMode="decimal"
                step="0.01"
            ...
            <section className="space-y-3">
              ...
              <div className="flex items-center justify-between text-xs text-slate-400">
                <span>Этот текст увидит только после успешной оплаты.</span>
            ...
            <section className="space-y-3">
              <Label icon={Wallet}>Куда получать</Label>
              <input
                type="text"
            ...
            <div className="pt-2">
              ...
              <p className="mt-3 text-center text-[11px] text-slate-400 leading-relaxed">
                Создавая счёт, вы соглашаетесь: платформа только обеспечивает оплату и не гарантирует доставку товара.
              </p>
            </div>
          </form>
        </div>

        <div className="mt-6 rounded-2xl bg-white border border-slate-200 p-4 flex items-start gap-3">
          <CheckCircle2 className="w-4 h-4 mt-0.5 shrink-0 text-emerald-500" />
          <p className="text-xs text-slate-500 leading-relaxed">
            После создания вы получите отдельную ссылку для себя — с QR-кодом, кнопкой «Поделиться в Telegram»
            и ожиданием оплаты. Покупатель получит ссылку на страницу оплаты.
          </p>
        </div>
      </div>
    </div>
