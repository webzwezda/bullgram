...
export function TonConnectPayButton({
  ...
  const { pay, paying, verifying, status, error, reset, verifyCurrent } = useTonCheckout({
    verifyEndpoint,
  ...
  const recovery = status === 'failed' || status === 'pending';
  const statusLabel = (() => {
  ...
  return (
    <div className={`flex flex-col items-center gap-2 ${fullWidth ? 'w-full' : ''} ${className}`}>
      ...
      {recovery && !busy ? (
        ...
        >
          Платёж не ушёл — оплатить заново
        </button>
      ) : null}
    </div>
  );
