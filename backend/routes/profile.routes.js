...
export default function profileRoutes(supabase) {
    ...
    router.get('/telegram/status', authenticateUser, async (req, res) => {
        try {
            ...
            // supabase-js реджектит при сетевом фейле — без try/catch это
            // Сетевой reject supabase-js заворачиваем в JSON-ошибку {error} — единый
            // контракт ошибок этого файла, как в остальных роутах.
            return res.status(500).json({ error: error?.message || 'Не удалось получить статус Telegram.' });
        }
    });
    ...
    router.delete('/telegram', authenticateUser, async (req, res) => {
        ...
        try {
            ...
            return res.status(500).json({ error: err?.message || 'Не удалось отвязать Telegram.' });
        }
    });
    ...
    router.put('/wallet', authenticateUser, async (req, res) => {
        try {
            ...
            return res.status(500).json({ error: 'Ошибка сети, попробуй ещё раз' });
        }
    });
