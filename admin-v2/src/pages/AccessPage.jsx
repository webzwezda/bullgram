...
export function AccessPage() {
  ...
  function handoffSingleIssue(row) {
    openUserbotCenterHandoff({
      ...
      draftMessage: row.status === 'active' && !row.last_join_approved_at
        ...
        : 'Привет. Пишу по доступу в Bullgram. Напиши, что сейчас не сходится, и я быстро разберу ситуацию.'
    });
  }
  ...
  function handoffBulkIssues(rows) {
    openBroadcastManualSelection(
      rows,
      'Доступ: ручной хвост',
      'Привет. Пишу по доступу в Bullgram. Если у тебя что-то зависло между оплатой и входом, ответь одним сообщением, и я быстро доведу до результата.'
    );
  }
