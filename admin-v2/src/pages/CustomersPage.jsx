            ...
            <>
                      ...
                      return (
                        <tr key={`${activeTab}-${row.id}`} className="hover:bg-slate-50/80 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <div className="font-black text-slate-900 text-sm truncate">{row.tg_username ? `@${row.tg_username}` : row.tg_user_id ? `ID: ${row.tg_user_id}` : 'Неизвестный'}</div>
                                {row.created_at && (
                          ...
                          <td className="px-6 py-4 hidden md:table-cell">
                            <div className="font-bold text-slate-800 text-sm mb-1 truncate">{row.title || row.channel_title || '—'}</div>
                            {row.channel_title && (
              ...
              {activeRows.length > limit && (
                <div className="px-8 py-4 border-t border-slate-100 bg-slate-50/30">
                  <button
                    className="w-full px-6 py-3 bg-white border border-slate-200 text-slate-700 rounded-xl text-sm font-bold shadow-sm hover:bg-slate-50 hover:border-slate-300 transition-all flex items-center justify-center gap-2"
