...
export function CrmPage() {
  ...
  function handoffSingleSubscriber(row) {
    openUserbotCenterHandoff({
      ...
      draftMessage: row.status === 'active' && row.in_group === false
        ...
        : 'Привет. Пишу по твоей подписке в Bullgram. Напиши, что сейчас нужно: доступ, продление или проверка статуса.'
    });
  }
  ...
  function handoffBulkSubscribers(rows) {
    openBroadcastManualSelection(
      rows,
      'CRM: ручной хвост',
      'Привет. Пишу по подписке в Bullgram. Если вопрос еще актуален, ответь одним сообщением, и я быстро разберу доступ или продление.'
    );
  }
