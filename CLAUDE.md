...
## Codex Workflow Notes (Removable)

This project uses Codex with cmux orchestration. If using Claude as a worker under Codex:
- Codex owns scope, integration, verification, deploys
- Claude workers receive bounded prompts with exact scope and allowed write sets
- Codex must inspect diffs before merging
- Use `.claude/agents/bullrun-codex-worker.md` for project-specific worker behavior
- cmux commands: `cmux tree`, `cmux list-panes`, `cmux new-split`, `cmux send`, `cmux read-screen`
