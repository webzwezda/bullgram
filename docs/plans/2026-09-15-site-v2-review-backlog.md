...
| 6 | батч-рантайм: редиректы, 404, hero-контент против правила «ОДИН исход», финальная дизайн-прогонка | **done (АУДИТ ЗАВЕРШЁН)** |
...
### Волна 6 — финальная зачистка — ЗАКРЫТА, аудит site-v2 ЗАВЕРШЁН

Сделано:
- site.css ужат 2430 → 26 строк (все 260 классов легаси-дизайна проверены grep'ом
  по jsx/js/html — ноль ссылок; токены Card живут в tailwind.css @theme, НЕ в site.css)
- удалены мёртвые файлы: material-symbols-outlined.woff2 (1.1MB), hero-telegram-mock.svg,
  components/ui/button.jsx; dep @fontsource-variable/geist снят (lockfile синхронизирован)
- rose→red во ВСЕХ ошибочных состояниях пяти поверхностей (700-shade для мелкого
  текста — контраст сохранён)
- ты-голос унифицирован (CreateInvoice, HomePage: «Заполни», «Отправь», «Укажи»,
  «Войди», «Напиши»); ПРАВИЛО ГОЛОСА: site-v2 — ты везде, КРОМЕ /access-request —
  там вы (уважение к уязвимой аудитории, решение волны 4)
- Pro-буллеты: исход «уже готовый к работе тг-аккаунт» первым, «безлимит по API и
  MCP» вторым (правило «ОДИН исход»)

Не делалось (осознанно):
- Manrope так и не подключен — сайт рендерится системным стеком с самого начала;
  подключить/удалить — решение владельца (файлы на месте)
- финальная визуальная прогонка владельцем: wave-6 дельта крошечная (порядок
  буллитов, цвета ошибок, голос), последние ACCEPT в силе; владелец был активен
  в браузере — табы не трогали

## Итог аудита site-v2 (2026-09-15)

6 волн закрыты за один день. Продуктовые: 1 P0-уровень (markExpired/updated_at —
500 на путях истечения счетов), 3 P1 денег (двойная оплата после неудачной verify,
поздние оплаты терялись в billing, вечный reload успеха), 2 P1-инфра (rate-limit по
спуфабельному XFF, implicit OAuth), SEO/OG с нуля до полного базового набора.
Все деплои push-to-main, смоки зелёные, тест-данные убраны, тестовые проверки —
живыми вызовами против прода.

Осталось владельцу (не блокеры):
1. ~~og:image~~ — СДЕЛАНО 2026-09-16: og-image.jpg 1200×630 сгенерирован из живого
   скриншота героя (кроп без сайдбара), og:image + summary_large_image на проде,
   подтверждено curl'ом
2. ~~шрифт~~ — СДЕЛАНО 2026-09-16: Manrope подключена через @font-face (latin+cyrillic,
   variable 200-800 — файлы сверены с Google Fonts по размеру подмножеств, font-display
   swap), рендер проверен визуально: геометрические литеры, чистый ExtraBold без
   синтетики, вёрстка не поехала. woff2-файлы оставлены (теперь используются)
3. admin-поверхность для access_requests (кто видит ПИИ) — остаётся; триггер: первая
   реальная заявка или признаки сбоев TG-уведомлений
4. диплинки кошельков проверить на реальных устройствах — смягчено 2026-09-16: QR
   консолидирован на стандартный ton:// (Trust-формат с непроверенным memo убран из
   QR-пути; ссылка Trust осталась в списке — при непроходе memo сработает amber-варнинг)
5. ~~PKCE-смоук~~ — СДЕЛАНО 2026-09-16 серверно: GoTrue валидирует code_challenge
   (43-128 симв., S256) и оба провайдера отдают PKCE-редиректы — Google через
   response_type=code, custom:telegram прямо несёт code_challenge в location. Логин
   владельца не затронут (проверка curl'ом, не кликом)

## Решения 2026-09-16 (пятое-шестое продолжение аудита)

- og:image сгенерирован из живого прод-скриншота (sips cropOffset 430 787, -c 1382 2633,
  -z 630 1200, jpeg q82, 107KB) — без ПИИ и браузерного хрома
- Manrope: файлы сверены с fonts.gstatic.com variable-подмножествами (latin 24836 vs
  24576 байт — совпадение), объявлен диапазон 200-800; font-black (900) безопасен —
  браузер берёт ближайший 800 без синтетики
- QR оплаты: один QR формата ton://transfer (standard, любой TON-кошелёк); переключатель
  Trust/TON QR удалён; deep-link список не тронут
- Правило голоса зафиксировано: site-v2 — ты-форма везде, кроме /access-request (вы —
  уважение к уязвимой аудитории)
