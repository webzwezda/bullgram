...
**Осталось вне этого этапа (см. BACKLOG):** порядок секций на /userbots (центр ниже онбординга независимо от состояния), мобильный горизонтальный скролл журнала вызовов (статус за свайпом), e2e покупки за TON на живых деньгах, скан ответов рассылки (CampaignReplies) живой кампанией. Закрыто хотфиксом: Telegram Web открытие из обоих приложений живым кликом (см. «Хотфикс» ниже; слепой пункт — алиас проверялся curl 200, т.е. nginx, а не приложение).

**Хотфикс 2026-09-26 (после ревью): Telegram Web на новом пути.** `window.open` в userbot-v2/admin-v2 переехал на `/userbot/telegram-web/<id>`, но парсер в userbot-web (`patches/src/util/bullgramBridge.ts` → `extractUserbotIdFromUrl`) ждал только `/app/telegram-web/` — любое открытие из UI показывало «В URL не указан id юзербота». Фикс: regex принимает оба префикса (старый остаётся живым для закладок). Попутно найдена причина, почему прод неделю держал июльский билд: `set -euo pipefail` + `ls -lh | head -10` в конце `userbot-web/scripts/build.sh` — `ls` умирал от SIGPIPE на большом dist, деплой-скрипт валился (exit 141) до rsync; строка декоративная, добавлен `|| true`. Деплой: `DEPLOY_HOST=srv ./ops/scripts/deploy-telegram-web.sh` (CI telegram-web не катит). Проверено в реальном Chrome: оба пути рендерят Telegram Web с живой сессией юзербота; переходные ошибки консоли (bridge 4429 RATE_LIMITED) — шум переподключения при смене пути в той же вкладке.

**Переименование 2026-09-26 (решение владельца): `/userbot/userbots` → `/userbot/accounts`.** Владельцу мешало дублирование «userbot/userbots» в URL. Новый путь `/userbot/accounts` (по сущности tg_accounts), лейбл навигации «Юзерботы» не менялся. Обновлены все deep-links (backend dashboard-сигналы, inbox-watcher, текст MCP-ошибки safe-mode), ссылки admin-v2 (CommandCenter, OpsRail, Broadcast, Audience, Customers-хандофф с `?tg_user_id=`, Referrals, MyPurchases), редиректы `/app/userbots` и `/app/bots` → `/userbot/accounts`, site-v2 `/shop` → `/userbot/accounts`. В userbot-v2 оставлен совместимый редирект `/userbots` → `/accounts` с прокидом query — старые ссылки уже ушли в Telegram-уведомления.

**Волна критика 2026-09-26 (вечер): парность оболочек.** Критик по 4 прод-кадрам подтвердил P0-претензию владельца: правый рельс в /userbot был выключен сознательно при копировании (комментарий «рейл — про paywall, сюда не копируется» + инлайн-override колонок в App.jsx), профиль с тарифом жил в левом сайдбаре, на мобайле — за бургером; гейт ui-sync оболочку не видит, поэтому дрейф дошёл до прода. Фикс 4547cf6: OpsRail получил проп showPaywall (paywall-секции — только /app, по правилу разделения продуктов), байт-идентичный порт в userbot-v2, блок личности из сайдбара удалён, футер «На сайт · Кабинет», инлайн-override убран; OpsRail добавлен в SHARED_FILES ui-sync (7/7); ratchet baseline обновлён осознанно (рост — только байт-идентичный порт, обоснование в коммите). Остатки критика (P1 пустой Command Center, P2 заголовок «BotFather-боты» и мобильный порядок рельса, P3 состав src/ui) — BACKLOG #20.

**Урок:** разведка «3 строки бэкенда» оказалась 10 (grep по всем маршрутам обязателен); npm run script --flag не пробрасывает флаг — гейтовые флаги гонять напрямую `node design/…`. Дополнение: URL-зависимые парсеры в вендорном форке (userbot-web) — отдельная зона переезда: `grep -rn "location.pathname"` по патчам форка обязателен, когда меняется префикс пути; HTTP 200 от nginx не доказывает работу приложения.
