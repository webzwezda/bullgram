...
### Волна 1 — DTCG-токены (2026-09-16, завершена)
- Сделано: `design/tokens/*.json` — 6 файлов, 259 токенов: 134 примитива (12 палитр, реально встречающихся в коде, байт-в-байт из theme.css, + white/black), 29 semantic-алиасов (все студийные правила), typography с машинным слоем интерлиньяжей `font.sizeLineHeight` (13), spacing 34, radius 10, motion 4; `design/README.md` — архитектура, железное правило, таблица «токен → правило», контракт маппинга в @theme
- Приёмка оркестратора: `validate_tokens` 6/6, exit=0; собственная байт-сверка 132 цветовых примитивов и 13 интерлиньяжей с theme.css — 0 расхождений; собственная жёсткая проверка алиасов поймала 1 висячий (`{typography.letterSpacing.widest}` — валидатор кита его прощал), исправлен до код-ревью
- Код-ревью: **ACCEPT**; 2 P2 закрыты сразу же (P2-1 маппинг DTCG→@theme в README — защита от коллизии `--text-*` с namespace размера шрифта; P2-2 машинный слой интерлиньяжей), 4 P3 закрыты (точность описаний: целевые стеки шрифтов, surface.page и тёплый фон site-v2, нестандартность композита micro-label, оговорки spacing/radius)
- Известное, не блокер: `border.strong`=slate-300 FAIL WCAG 1.4.11 (1.42:1) — де-факто студии; кандидат на ужесточение slate-400/500 — отдельное визуальное решение владельца. Все обязательные AA-пары проходят; `ink.muted` 4.76:1 на белом — floor держится
- В волну 2: build-скрипт обязан читать «Маппинг в @theme» из README; hex-слепок для `validate_contrast` сделать постоянным скриптом, а не одноразовым

### Волна 2 — пайплайн (2026-09-16, завершена)
- Сделано: `design/build.mjs` — DTCG → байт-идентичные `tokens.css` обоих приложений (один `@theme`, 209 переменных; контракт README исполнен, `--font-sans` сознательно не эммитится до волны 4); `npm run tokens:build` / `tokens:check` (stale-гейт + гейт нулевой дельты: 132 color-значения байт-равны theme.css); `design/contrast-snapshot.mjs` — oklch→sRGB слепок для validate_contrast (round-trip на 4 якорях)
- Приёмка оркестратора: tokens:check моим прогоном зелёный; копии байт-идентичны; запрещённые эммиты отсутствуют; spot-check семантики (`--color-ink-muted`, `--color-action-ton`, `--text-sm--line-height`) сошёлся
- Код-ревью: **ACCEPT**; ревьюер атаковал гейт в песочнице — подмена значения, ручная правка, BOM/CRLF, битые алиасы ловятся все. P2-1 «гейт не проверял полноту» (удаление палитры проходило зелёным) закрыт ассертом `checked === 132` до старта волны 3 — негативный тест воспроизведён оркестратором в песочнице (teal → exit 1, «сверено 121, ожидается 132»). P3 закрыты: try/catch на ошибках алиасов, две строки README (пометка про --font-sans с волны 4, относительный путь слепка)
- В волну 3: подключение импорта в оба приложения, билды, baseline-скриншоты прода ДО деплоя

### Волна 3 — admin-v2: подключение токенов (2026-09-16, завершена)
- Сделано: `@import "./tokens.css"` в admin-v2 tailwind.css (после tailwindcss — палитры переопределяют дефолты); деплой 9491efa, CI + смоук зелёные
- Доказательство нулевой дельты: (1) детерминированное — tokens:check 132 значения байт-равны theme.css; (2) рантаймное — baseline/after скриншоты 4 ключевых экранов (command center, customers, broadcast, userbots, 1920×1080) сравнены попиксельно: **diff bbox=None на всех парах — ни один пиксель не сдвинулся**
- Подтверждено в бандле: `.text-slate-500{color:var(--color-slate-500)}` — сырые классы идут через наши токены; `--font-sans` остался Geist (фонт-переменные не эммитятся до волны 4); семантические переменные tree-shaken до первого использования — штатное поведение v4
- design-critic не требовался: визуального изменения нет по построению и доказано попиксельно

### Волна 4 — шрифт и компонентный слой (2026-09-16, завершена: f578c91 + 3d205a9)
- 4a: Manrope self-hosted в admin-v2 (`/app/manrope-*.woff2` — утверждённое отклонение от буквы ТЗ: у admin vite base '/app/', дословный '/' тянул бы шрифт из site-v2); @font-face сплит cyr/lat; эммит `--font-sans`/`--font-mono` включён в build.mjs (211 переменных); каскадная война снесена (font-family из app.css :root удалён — теперь токен → @theme → tailwind base); hex→var 24 замены строго при байт-равенстве (85 legacy-hex задокументированы как нетронутые); mono-стек унифицирован ×3; tabs.jsx → cva (классы байт-идентичны); Geist снят с зависимости и из кода (0 вхождений)
- 4b: DESIGN.md переписан на семантику токенов с таблицей соответствия старых правил; три коррекции значений помечены явно (amber-800→700, rose→red, helper slate-400→500 = floor ink.muted)
- Приёмка оркестратора: tokens:check 211 vars зелёный; woff2 байт-в-байт из site-v2; рантайм прода: стек токена рендерится на body/headings, Manrope [loaded] ×2 из /app/ (200 font/woff2), Geist 0; lockfile синхронизирован
- Код-ревью: **ACCEPT**, все находки P3; перенос в волну 6: самобост JetBrains Mono, preload/size-adjust fallback (CLS на холодной загрузке), mono-стек → var(--font-mono); ₽ вне unicode-range — косметика (стандартный fontsource-сплит)
- Дизайн-критик: **ACCEPT на все 4 экрана — «ship it»**: диф 1.5–2.6% только контурное перо глифов (вебшфт против локального файла), вёрстка неподвижна (контрольные абзацы ломаются на те же строки), кириллица из Manrope, правила студии на месте; все issues minor и до-волновые
- Смысл волны закрыт: владельцу картинка не изменилась (у него уже была локальная Manrope), а всем остальным вместо случайного системного фолбэка — детерминированный брендовый шрифт

### Волна 5 — гейты качества (2026-09-16, завершена: 5cb6c5d)
- Сделано: `npm run check:design` — гейты: tokens / contrast (известный FAIL border.strong толерируется по точной метке, любой новый — красный) / hardcodes-ratchet (baseline 5301, красный только при росте total+kind+file, `--update-baseline` осознанная) / axe на harness / ui-sync
- Нюанс: harness с `--qa-*` слоем (sRGB-слепок) — kit measure_render не парсит oklch computed styles
- Приёмка: свой зелёный прогон + повтор red-теста (probe-hex в CustomersPage → красный с точным дифом 5302>5301 → откат)
- Ревью: ACCEPT + 2 P2 «ложно-зелёный при дрейфе формата валидаторов» — фэйлсейфы внесены сразу (contrast: exit≠0 без распознанных FAIL-пар = красный; hardcodes: без «Scanned N» = красный и отказ перезаписи baseline)

### Волна 6 — перенос site-v2 (2026-09-16, завершена: cbd6a38 + f7e67a7 + 11142cf + f4349b3)
- 6a: tokens.css в site-v2 (тёплый фон и локальный @theme сохранены); button/input/badge вокендорены байт-в-байт из admin-канона; акцент действий blue→indigo (лендинг 7 элементов, AccessRequest, ErrorBoundary); sky-TON и декоративный blue сохранены; site.css удалён вместе с асинхронным импортом (гонка стилей закрыта)
- 6b: preload Manrope ×2; JetBrains Mono самобост (алиасные @font-face — пакет регистрирует 'JetBrains Mono Variable', стеки зовут 'JetBrains Mono'); mono ×3 → `var(--font-mono, fallback)`; ui-sync 5-м гейтом (5 файлов, sha256, негативно протестирован); baseline 5301→5298
- Инцидент инфраструктуры: два падения деплоя вскрыли, что условный npm install был мёртвым кодом — re-exec в deploy-pull.sh обнулял diff (PREV_HEAD==NEW_HEAD во втором проходе; env между проходами не переживает обёртку). Фикс: PREV_HEAD через файл в `.git/`; JBM переведён с npm-пакета на woff2 в public/fonts, чтобы деплой не зависел от install. Прод весь инцидент работал (аборт до pm2 reload)
- Рантайм прода: обе первичные кнопки — один пиксель #4c3aed; JBM loaded из self-hosted; Manrope 2/2
- Критик: ACCEPT на оба экрана; MAJOR-замечание (в H1 blue-слово «работают» рядом с индиго-градиентом) и minor (активная навигация blue — интерактивное состояние) исправлены сразу (f4349b3), проверены на проде
- Хвост на будущее (осознанно не делаем): JBM/preload для site-v2 (сайт на дефолтном font-mono — дрейф, который ui-sync не ловит — кандидат в мини-волну); hover/focus-состояния не снимались

### Волна 7 — закрытие (2026-09-16, завершена)
- Governance: `design/CHANGELOG.md` (SemVer: значение примитива = MAJOR, новый токен = MINOR, доки = PATCH; компонент в канонический кит при ≥2 использованиях), ссылка из README
- **ТЕСТ ЕДИНСТВА ПРОЙДЕН**: смена indigo.600 → `tokens:build` → оба tokens.css обновились синхронно → откат → дерево чистое, гейт зелёный. Слово «единая» доказано механически
- Финальный `check:design`: PASS 5/5

### Критерий приёмки цели (5.5) — финальный статус
1. Один DTCG-источник: ✅ design/tokens/*.json, 259 токенов
2. Оба приложения рендерят палитры из tokens.css: ✅ (admin — волна 3, попиксельная нулевая дельта на 4 экранах; site — волна 6a)
3. Один шрифт: ✅ Manrope self-hosted в обоих (админ — волна 4, критик «ship it»)
4. Действия: ✅ action.primary=indigo в обоих, action.ton=sky изолирован, реальный дрейф убран
5. Гейты ловят регресс: ✅ check:design 5/5, red-green подтверждён дважды (оркестратором и ревьюером)
6. CHANGELOG и SemVer: ✅ design/CHANGELOG.md
